"""OPTIMIZED Census ACS Text2SQL agent -- same retrieval, far fewer tokens (Option A).

This is a sibling of `census_agent.py` (which is left untouched). It keeps the
exact table-level disambiguation that already works (Neocarta table embeddings
pick the right geography/year/span out of 278 near-identical tables), but it
COMPACTS the schema payload before it ever reaches the model.

Why this saves tokens:
  The raw Neocarta tool `get_context_by_table_hybrid_search` returns the correct
  table with ALL ~252 columns, and for the Census graph every column is serialized
  with null/empty fields:
      {"column_name":"median_age","column_description":null,"data_type":"FLOAT64",
       "nullable":true,"examples":[],"key_type":null,"references":[]}
  ~85% of that ~9.8K-20K-token payload is JSON scaffolding carrying no signal.
  It is then re-sent as model input on every later turn, which is how WITH-mode
  reaches ~37K tokens/question.

  This agent wraps that same retrieval in `get_table_schema_compact`, which keeps
  only `column_name` + `data_type` (plus description / FK refs when they exist) and
  returns a terse line. Same table accuracy, a fraction of the tokens.

Usage:
    ./.venv/bin/python module-3/census_agent_optimized.py                 # interactive, WITH (compact)
    ./.venv/bin/python module-3/census_agent_optimized.py --mode without  # brute-force baseline
    ./.venv/bin/python module-3/census_agent_optimized.py --runall        # sweep + comparison table

Compare against the original by running `census_agent.py --mode with` (raw payload)
and this `census_agent_optimized.py --mode with` (compact payload) on the same
question -- the TOKEN TRACE shows the tool-result size collapse.
"""
...
if os.getenv("CENSUS_QUIET", "1").lower() not in {"0", "false", "no"}:
    STDERR_LOG = os.getenv("CENSUS_STDERR_LOG", "/tmp/census_agent_optimized.stderr.log")
...
# The underlying Neocarta retrieval we wrap. We expose our compact wrapper to the
# agent instead of the raw tool, so the agent never sees the 252-column null dump.
...
def _extract_json_text(raw) -> str:
    """Neocarta tool results arrive either as a JSON string or as a list of
    content blocks like [{"type":"text","text":"[...]"}]. Normalize to the text."""
...
# Whether to keep per-column descriptions. Off by default: for the Census graph
# these are mostly just the title-cased column name (e.g. nonfamily_households ->
# "Nonfamily households"), so they ~5x the payload for almost no extra signal.
# Turn on (CENSUS_COMPACT_DESCRIPTIONS=1) if a measure is genuinely ambiguous.
INCLUDE_COL_DESCRIPTIONS = os.getenv("CENSUS_COMPACT_DESCRIPTIONS", "0").lower() in {"1", "true", "yes"}


def compact_table_context(raw) -> str:
    """Strip the null/empty boilerplate from a Neocarta TableContext payload.

    Keeps table name + description and a terse `name type` per column (FK refs are
    kept when present; column descriptions only when CENSUS_COMPACT_DESCRIPTIONS is
    set). This is the whole Option-A token win: identical retrieval, ~85% less
    payload that then gets re-sent on every downstream model call."""
    text = _extract_json_text(raw)
    try:
        data = json.loads(text)
    except Exception:  # noqa: BLE001 — if it isn't JSON, pass it through untouched
        return text
    tables = data if isinstance(data, list) else [data]
    if not tables:
        return "No matching table found. Refine the geography/year/measure terms."
    blocks: list[str] = []
    for t in tables:
        ...
        for c in cols:
            ...
            if not name:
                continue
            piece = f"{name} {c.get('data_type') or ''}".strip()
            desc = c.get("column_description")
            ...
            parts.append(piece)
        blocks.append(
            f"TABLE {t.get('table_name')} — {t.get('table_description', '')}\n"
            f"  {len(parts)} columns: " + ", ".join(parts)
        )
    return "\n\n".join(blocks)


# Hard cap on tables returned. Table embeddings reliably rank the correct
# geography/year/span table #1 (verified 4/4), so we force a single table inside
# the wrapper rather than exposing max_tables to the model -- otherwise the agent
# sometimes pulls 5-6 tables and re-inflates the payload it was meant to shrink.
COMPACT_MAX_TABLES = int(os.getenv("CENSUS_COMPACT_MAX_TABLES", "1"))


def make_compact_retrieval_tool(neocarta_table_tool: BaseTool) -> BaseTool:
    """Wrap the raw Neocarta table search so the model receives a compact schema."""
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "Find the single best-matching Census ACS table for the question and "
            "return its name plus a COMPACT column list (column name + data type). "
            "Pass the key terms from the question (geography level + year + survey "
            "span + the measure) as text_content. Call this exactly once."
        ),
    )
...
* Call get_table_schema_compact ONCE with the key terms from the question
  (geography level + year + survey span + the measure). It returns the single
  best-matching table and a compact list of its columns (name + type).
* Do not call it again. One targeted retrieval returns everything you need.
Then call execute_sql.

Rules:
* Never SELECT * (tables have ~252 columns). Select only the columns you need.
...
def print_trace(messages: list) -> None:
    """Show where the tokens go: size of each message, and how the tool results
    ...
    print("  -> a big tool result is re-sent as input on every later model call, so")
    print("     input ~= (context size) x (number of model calls). Shrinking the tool")
    print("     result (compact schema) shrinks every downstream replay too.\n")
...
def build_tools_prompt(mode: str, neocarta_tools: list) -> tuple[list[BaseTool], str]:
    """Return (tools, system_prompt). WITH exposes ONLY the compact retrieval
    wrapper (+ execute_sql); WITHOUT uses brute-force BigQuery discovery."""
    if mode == "with":
        raw = next((t for t in neocarta_tools if t.name == NEOCARTA_TABLE_TOOL), None)
        if raw is None:
            raise RuntimeError(
                f"Neocarta tool '{NEOCARTA_TABLE_TOOL}' not registered. Is the `census` "
                "graph built with Table vector + full-text indexes?"
            )
...
def print_runall_table(rows: list[dict]) -> None:
    ...
    print("\n" + "=" * 104)
    print("  PER-QUESTION RESULTS  (WITH = compact retrieval)")
    print("=" * 104)
    ...
    print("\n  SAVING% = (without - with)/without per question (positive = compact-with cheaper).")
...
async def run_all(client: MultiServerMCPClient, args) -> None:
    ...
    print("\n" + "=" * 80)
    print("  RUN-ALL SWEEP (OPTIMIZED): models x questions x {without, with-compact}")
...
async def main() -> None:
    ...
    parser.add_argument("--mode", choices=["with", "without"], default="with",
                        help="with = compact Neocarta retrieval; without = brute-force discovery")
    ...
    parser.add_argument("--runall", action="store_true",
                        help="Non-interactive sweep over every model x demo question x mode.")
    ...
    banner = ("WITH Neocarta (COMPACT retrieval)" if args.mode == "with"
              else "WITHOUT Neocarta (brute-force discovery)")
    print("\n" + "=" * 80)
    print(f"  MODE : {banner}")
    print(f"  MODEL: {model}")
    ...
    while True:
        ...
        try:
            ...
            state = await agent.aget_state(config)
            last_messages = state.values.get("messages", [])
            print(f"\n[!] Hit the model provider's tokens-per-minute (TPM) cap mid-run "
                  f"({model}).\n    Options: use a cheaper/faster model, wait ~60s, or raise the "
                  "org's rate limit. (The partial tally below shows how far it got.)\n")
