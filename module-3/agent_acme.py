"""Interactive Text2SQL agent for the ACME Corp demo -- WITH vs WITHOUT Neocarta.

Self-contained demo/benchmark agent for the workshop's `acme_corp` dataset and the
`symantic` metadata graph from Module 2 (the same data 3_agent.py uses). It does NOT
modify 3_agent.py, measure_tokens.py, or census_agent.py.

The demo questions are the example questions from the repo's module-3 README.

Run the SAME question in each mode and watch the difference:

    # No semantic layer: the agent brute-forces the schema via INFORMATION_SCHEMA.
    ./.venv/bin/python module-3/agent_acme.py --mode without

    # With Neocarta: a targeted retrieval finds the right tables/columns/joins.
    ./.venv/bin/python module-3/agent_acme.py --mode with

    # Non-interactive sweep over models x questions x {with, without}:
    ./.venv/bin/python module-3/agent_acme.py --runall                 # all models w/ keys
    ./.venv/bin/python module-3/agent_acme.py --runall --model gpt-4o-mini

Options:
    --mode with|without   (default: with)
    --model <name>        LiteLLM id; omit for an interactive menu
    --runall              sweep all combinations and print a comparison table
    --no-trace            hide the per-message token breakdown (shown by default)

After every question it prints a tally (input/output tokens, est. cost, #tool calls,
tables queried) AND a per-message token breakdown showing where the tokens go.

Connection summary:
  * Metadata / semantic layer -> Neocarta MCP (stdio via uvx) against the `symantic`
    Neo4j database built in Module 2.
  * Query execution           -> a local, cost-capped BigQuery executor against
    <GCP_PROJECT_ID>.acme_corp.
"""
...
# Silence LiteLLM's "Give Feedback / Get Help" banner + info lines it prints to stdout
# on provider errors (e.g. an OpenAI 429 TPM rate limit). We surface our own concise
# message instead, so the demo / sweep output stays clean.
...
# Keep the demo terminal clean: the neocarta MCP subprocess + Neo4j driver write
# verbose notifications/warnings to stderr that look alarming but are NOT failures.
# Redirect this process's stderr (fd 2 -> a log file), which the child inherits. All
# demo output uses print() -> stdout and is unaffected. Set ACME_QUIET=0 to keep it.
STDERR_LOG: str | None = None
if os.getenv("ACME_QUIET", "1").lower() not in {"0", "false", "no"}:
    STDERR_LOG = os.getenv("ACME_STDERR_LOG", "/tmp/agent_acme.stderr.log")
    _errlog_fh = open(STDERR_LOG, "w", buffering=1)  # noqa: SIM115
    os.dup2(_errlog_fh.fileno(), 2)

BILLING_PROJECT = os.getenv("GCP_PROJECT_ID")
SOURCE_PROJECT = os.getenv("ACME_SOURCE_PROJECT", BILLING_PROJECT)
SOURCE_DATASET = os.getenv("ACME_SOURCE_DATASET", os.getenv("BIGQUERY_DATASET_ID", "acme_corp"))
NEO4J_DATABASE = os.getenv("ACME_NEO4J_DATABASE", os.getenv("NEO4J_DATABASE", "symantic"))
...
# LiteLLM retries on transient errors (e.g. rate limits). Lower it for a fast sweep so a
# doomed combo (e.g. gpt-4o vs a 30K TPM cap) doesn't grind through 8 backoffs:
#   ACME_NUM_RETRIES=1 ./.venv/bin/python module-3/agent_acme.py --runall
NUM_RETRIES = int(os.getenv("ACME_NUM_RETRIES", "8"))

# LLMs offerable in the startup menu: (litellm_model_id, label, required_api_key_env).
# Models whose key is missing from .env are hidden. Anthropic ids need the litellm
# "anthropic/" prefix; OpenAI ids are recognised as-is.
MODEL_MENU = [
...
# The example questions from the repo's module-3 README (Exploring the ACME Dataset).
DEMO_QUESTIONS = [
    "Which product line contributes the most to total ARR, and what is the breakdown?",
    "Which subscriptions are renewing in the next 90 days, ranked by ARR at risk?",
    "What is the total committed annual vendor spend by category?",
    "What is the win rate and average deal size by customer segment?",
    "How are performance ratings distributed across departments in the most recent review cycle?",
...
def execute_sql(sql: str) -> str:
    ...
    Tables must be fully qualified, e.g. `project.acme_corp.subscriptions`.
...
@tool
def list_acme_tables() -> str:
    """List all table names in the ACME Corp dataset."""
...
def get_table_columns(table_name: str) -> str:
    """Return the column names and data types for one ACME Corp table."""
    rows = _bq.query(
...
# Shared analytic rules (applied to BOTH modes so the comparison is about schema
# discovery, not a prompt advantage).
_RULES = """Rules:
* A business label (e.g. "product line") usually maps to a descriptive column on a
  DIMENSION table (e.g. products.name), NOT a free-text label on a fact/transaction
  table. Resolve IDs to human-readable names by joining the dimension table.
* When a question asks which entity "contributes the most", aggregate (GROUP BY + SUM)
  over the right dimension and join to resolve names -- do not just return the single
  largest row.
* Select only the columns you need. Give a short, readable final answer."""

PROMPT_WITH = f"""You are a Text2SQL agent for the ACME Corp B2B SaaS dataset in BigQuery.
Tables live in `{TABLE_FQN}` and must be fully qualified as `{TABLE_FQN}.<table>`.

The dataset spans customers, subscriptions, products, opportunities, invoices, support
tickets, employees, vendors and more. Business terms (ARR, churn, win rate, health
score, vendor spend, CSAT) map to specific tables/columns/joins.

MANDATORY before writing SQL:
* FIRST call a get_context_* metadata-retrieval tool with the key business terms from
  the question to fetch the relevant tables, columns, join keys, and business-term
  definitions from the semantic layer.
* THEN write SQL using exactly those tables/columns and call execute_sql.

{_RULES}"""

PROMPT_WITHOUT = f"""You are a Text2SQL agent for the ACME Corp B2B SaaS dataset in BigQuery.
Tables must be fully qualified as `{TABLE_FQN}.<table>`.

You have no semantic layer. Use list_acme_tables to see table names and
get_table_columns to inspect a table's columns. Discover the correct tables, columns,
and join keys, then call execute_sql.
        ...
        if isinstance(m, AIMessage):
            for tc in m.tool_calls or []:
                ...
                if tc["name"] == "execute_sql":
                    tables.update(re.findall(rf"{re.escape(SOURCE_DATASET)}\.([a-zA-Z0-9_]+)", tc["args"].get("sql", "")))
...
def print_trace(messages: list) -> None:
    """Show where the tokens go: size of each message, and how big tool results get
    re-sent as model input on every subsequent call (stateless replay)."""
...
def make_mcp_client() -> MultiServerMCPClient:
    """Neocarta MCP (stdio) against the ACME (`symantic`) Neo4j database."""
...
def build_tools_prompt(mode: str, neocarta_tools: list) -> tuple[list[BaseTool], str]:
    ...
    targeted neocarta retrieval tools (any get_context_* variant -- this graph has the
    business-term hybrid tools); never the full-schema-dump / list_* tools. WITHOUT uses
    brute-force BigQuery discovery."""
    if mode == "with":
        retrieval = [t for t in neocarta_tools if t.name.startswith("get_context")]
        tools: list[BaseTool] = retrieval + [execute_sql]
        prompt = PROMPT_WITH
    else:
        tools = [list_acme_tables, get_table_columns, execute_sql]
...
def print_runall_table(rows: list[dict]) -> None:
    ...
    hdr = (f"  {'MODEL':<18}{'MODE':<9}{'Q':<3}{'IN':>9}{'OUT':>7}{'TOTAL':>9}"
           f"{'COST':>10}{'CALLS':>6}{'OK':>4}  TABLES")
    print(hdr)
    print("  " + "-" * (len(hdr) - 2))
    for r in rows:
        tbl = (",".join(r["tables"]) or "-")[:34]
...
async def run_all(client: MultiServerMCPClient, args) -> None:
    ...
    print("\n" + "=" * 80)
    print("  RUN-ALL SWEEP: models x questions x {without, with} Neocarta  [ACME Corp]")
    print(f"  MODELS   : {[short_model(m) for m in models]}")
    print(f"  QUESTIONS: {len(DEMO_QUESTIONS)}   GRAPH: `{NEO4J_DATABASE}`   DATA: {TABLE_FQN}")
    print(f"  TOTAL RUNS: {len(models) * 2 * len(DEMO_QUESTIONS)}")
    if STDERR_LOG:
...
async def main() -> None:
    ...
    parser.add_argument("--model", default=None,
                        help="LiteLLM model id (e.g. gpt-4o, anthropic/claude-sonnet-4-5-20250929). "
    ...
    print(f"  MODE : {banner}")
    print(f"  MODEL: {model}")
    print(f"  GRAPH: Neo4j db `{NEO4J_DATABASE}`  |  DATA: {TABLE_FQN}")
    print(f"  TOOLS: {[t.name for t in tools]}")
    if STDERR_LOG:
        print(f"  (server/driver logs -> {STDERR_LOG}; set ACME_QUIET=0 to show on screen)")
    print("=" * 80)
    print("\nDemo questions you can paste:")
    for i, q in enumerate(DEMO_QUESTIONS, 1):
        print(f"  {i}. {q}")
    print(f"\nType a question (or 1-{len(DEMO_QUESTIONS)} to use a demo question), 'exit' to quit.\n")
    ...
    while True:
        ...
        try:
            ...
            state = await agent.aget_state(config)
            last_messages = state.values.get("messages", [])
            print(f"\n[!] Hit the model provider's tokens-per-minute (TPM) cap mid-run on {model}.\n"
                  "    Options: use --model gpt-4o-mini (higher TPM), wait ~60s, or raise the org's "
                  "rate limit. (The partial tally below shows how far it got.)\n")
